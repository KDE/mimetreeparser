                                                  ...
                                                  QList<MessageWidgetContainer *> &containerStack)
{
    for (int i = 0, count = parts->rowCount(parent); i < count; i++) {
        ...
        while (!containerStack.isEmpty()) {
            const auto containerPart = containerStack.last()->containerPart();
            if (part->encryptionPart() == containerPart->encryptionPart() && part->signaturePart() == containerPart->signaturePart()) {
                reuse_container = true;
                break;
            }
            if (part->isChildOf(containerPart)) {
                break;
            }
