...
class ItineraryMemento::Private
{
public:
...
ItineraryMemento::ItineraryMemento()
    : d(std::make_unique<ItineraryMemento::Private>())
{
    d->m_postProc.setValidationEnabled(false);
}

ItineraryMemento::~ItineraryMemento() = default;

const std::vector<ItineraryMemento::DocumentData> &ItineraryMemento::documentData() const
{
    return d->m_docs;
}

const std::vector<ItineraryMemento::PassData> &ItineraryMemento::passData() const
{
    return d->m_passes;
}
...
QVector<ItineraryMemento::TripData> ItineraryMemento::data()
{
    if (d->m_data.isEmpty() && !d->m_postProc.result().isEmpty()) {
        ...
        auto postProcResult = d->m_postProc.result();
        ...
        const KCalendarCore::MemoryCalendar::Ptr calendar(
            new KCalendarCore::MemoryCalendar(QTimeZone())); // CalendarSupport::calendarSingleton(!qEnvironmentVariableIsSet("BPF_ITINERARY_TESTMODE"));
...
{
    if (index >= d->m_data.size()) {
        return;
    }
    d->m_data[index].expanded = !d->m_data.at(index).expanded;
}

void ItineraryMemento::addPass(KPkPass::Pass *pass, const QByteArray &rawData)
{
    d->m_passes.emplace_back(PassData{pass->passTypeIdentifier(), pass->serialNumber(), rawData});
}

QByteArray ItineraryMemento::rawPassData(const QString &passTypeIdentifier, const QString &serialNumber) const
{
    for (const auto &pass : d->m_passes) {
...
{
    d->m_docs.push_back({docId, docInfo, docData});
}

bool ItineraryMemento::canAddToCalendar() const
{
    for (const auto &d : d->m_data) {
