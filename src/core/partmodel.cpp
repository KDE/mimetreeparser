...
{
    const auto signaturePart = messagePart->signature();
    if (!signaturePart) {
        return std::nullopt;
    }
...
SignatureInfo encryptionInfo(MimeTreeParser::Core::MessagePart *messagePart)
{
    ...
    const auto encryptionPart = messagePart->encryption();
    if (encryptionPart) {
...
QVariant PartModel::data(const QModelIndex &index, int role) const
{
    ...
    if (index.internalPointer()) {
        ...
        case EncryptionSecurityLevelRole: {
            ...
            return messagePart->encryption() ? SecurityLevel::Good : SecurityLevel::Unknow;
        }
        case EncryptionIconNameRole: {
            ...
            return messagePart->encryption() ? u"mail-encrypted"_s : QString();
        }
