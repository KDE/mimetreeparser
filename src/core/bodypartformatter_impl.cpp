...
contentsAsEncryptedMessagePart(ObjectTreeParser *objectTreeParser, KMime::Content *parent, const QList<KMime::Content *> &contents)
{
    if (contents.count() == 1 && contents[0]->contentType()->mimeType() == "application/pkcs7-mime"_ba) {
        return QSharedPointer<EncryptedMessagePart>(new EncryptedMessagePart(objectTreeParser, QGpgME::smime(), parent, contents[0]));
    }
    ...
    if (contents.count() == 2 && contents[0]->contentType()->mimeType() == "application/pgp-encrypted"_ba
        ...
        return QSharedPointer<EncryptedMessagePart>(new EncryptedMessagePart(objectTreeParser, QGpgME::openpgp(), parent, contents[1]));
    }
...
public:
    QSharedPointer<MessagePart> process(ObjectTreeParser *objectTreeParser, KMime::Content *node) const override
    {
        ...
        QSharedPointer<EncryptedMessagePart> mp(new EncryptedMessagePart(objectTreeParser, QGpgME::openpgp(), node, data));
        return mp;
    }
};
...
public:
    QSharedPointer<MessagePart> process(ObjectTreeParser *objectTreeParser, KMime::Content *node) const override
    {
        ...
        if (!isSigned) {
            ...
            mp = QSharedPointer<EncryptedMessagePart>(new EncryptedMessagePart(objectTreeParser, QGpgME::smime(), node, node));
...
public:
    QSharedPointer<MessagePart> process(ObjectTreeParser *objectTreeParser, KMime::Content *node) const override
    {
        ...
        */
        ...
        return QSharedPointer<EncryptedMessagePart>(new EncryptedMessagePart(objectTreeParser, protocol, node, data));
    }
};
